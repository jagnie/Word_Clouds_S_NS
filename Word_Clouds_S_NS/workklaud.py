...
#spam_words = ' '.join(list(map(str,df[df['label'] == 0 ]['clean_emails'])))
spam_words = ' '.join(list(map(str,df['clean_emails'])))
timestr = time.strftime("%Y%m%d-%H%M%S")
wordcloud_spam = WordCloud(background_color="white",width=1600, height=800, max_words=300).generate(spam_words)
